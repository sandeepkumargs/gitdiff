...
const getTestScenario = async (storyId) => {
    ...
    if (!response.ok) {
        console.error('Failed to fetch test scenarios');
    }
    const data = await response.json();
    return data;
};
...
const processAllUserStories = async (userStories) => {
    const url = `${AC_URL}/ac/api/v1/process`;
    const response = await fetch(url, {
        method: 'POST',
        headers: getAuthHeaders(),
...
export const generateTestScenario = async (userStory) => {
    const url = `${TS_URL}/ts/api/v1/generate`;
    const response = await fetch(url, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify(userStory)
    ...
    if (!response.ok) {
        console.error('Failed to generate test scenarios');
    }
...
const refineUserStory = async (sessionId, userStory) => {
    ...
    if (!response.ok) {
        console.error('Failed to refine user story');
    }
...
export const getProjectMetrics = async (project_id) => {
    try {
        const response = await fetch(`${ORG_API_BASE_URL}/org/api/v1/project/metrics?project_id=${project_id}`, {
            method: 'GET',
            headers: getAuthHeaders(),
        });
        if (!response.ok) {
