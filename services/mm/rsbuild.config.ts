...
export default defineConfig({
  source: {
    define: {
      ...
      'process.env.PLANTUML_SERVER_URL': JSON.stringify(process.env.PLANTUML_SERVER_URL),
    },
  },
  plugins: [pluginReact()],
  server: {
  ...
  output: {
    assetPrefix: "http://49.249.95.65:3002",
  },
  tools: {
    rspack: {
      output: {
      ...
      plugins: [
        new ModuleFederationPlugin({
          name: 'mindmaps',
          exposes: {
            './mindmaps': './src/pages/index.tsx',
            './mindmapchecker':'./src/pages/mindmaps/index.tsx',
          },
          shared: {
